...
export default async function handler(req, res) {
  ...
  /* email validation */
  if (
    /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email.trim()) === false
  ) {
    /* 515 custom status - invalid email */
    res.status(512).json({ message: 'Invalid email.' });
    return;
  }
  if (
    username.trim() === '' ||
  ...
  if (existingEmail) {
    /* 515 custom status - I'm a teapot! */
    res.status(515).json({ message: 'Email already exists.' });
    client.close();
    return;
  }
